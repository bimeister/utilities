...
describe("resize-observable.function.ts", () => {
  ...
  it("should call method unobserve of ResizeObserver", () => {
    const element: HTMLElement = document.createElement("div");
    const unobserveMock: unknown = jest.spyOn(resizeObserverMock, 'unobserve');

    resizeObservable(element).subscribe().unsubscribe();
